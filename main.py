...
UPLOAD_FOLDER = os.path.realpath(os.environ.get('UPLOAD_FOLDER', '/tmp'))
...
def upload_file():
    ...
    if os.path.dirname(filepath) != upload_dir:
        logger.warning("Rejected upload with suspicious path from %s: %r", request.remote_addr, file.filename)
        return jsonify(error="Invalid filename"), 400
